...
def handle_submodels(selections, model_dict, dst_dir, inpainting=False):
    ...
    for selection_name in cleaned_selections:
        ...
        if selection_name == 'ALL':
            for model_group_item in model_dict.values(): # Changed loop variable name to avoid confusion
                # Normalize model_group_item to a list for iteration
                items_to_process = model_group_item if isinstance(model_group_item, list) else [model_group_item]
        ...
        if selection_name in model_dict:
            ...
            # Normalize model_group to a list for iteration
